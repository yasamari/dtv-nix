...
stdenv.mkDerivation rec {
  ...
  src = fetchFromGitHub {
    ...
    repo = "NVEnc";
    tag = version;
    hash = "sha256-iDTWYTPWFQbFKFZkcNuYbZFm0LtGoC6cmxRba1UafeQ=";
  ...
  postPatch = ''
    ...
    substituteInPlace meson.build \
      --replace-fail \
        ...
        "npp_search_dirs = [cuda_target_lib_dir, cuda_lib_dir, '${cudaPackages.libnpp.static}/lib', '${cudaPackages.cuda_cudart}/lib']"
  ...
  configurePhase = ''
    ...
    meson setup build \
      --buildtype release \
      --prefix "$out" \
      -Dnvenc_cuda_dir="${cudaPackages.cudatoolkit}" \
