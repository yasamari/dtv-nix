...
stdenv.mkDerivation rec {
  ...
  src = fetchFromGitHub {
    ...
    tag = version;
    hash = "sha256-SuP3BIuPeeJWrC0AMO1rJEriNeC+cksARPNXDeRyRYw=";
  ...
  postPatch = ''
    substituteInPlace meson.build --replace-fail "cuda_dep = dependency('cuda', version : '>=11.0', required : true, static : true)" "cuda_dep = declare_dependency()"
  '';
